//! MSI (high-speed internal) clock driver for the STM32WL5x family. [^doc_ref]
//!
//! # Usage
//!
...
//! ```rust,ignore
//! let msi = &peripherals.stm32wl5.clocks.msi;
//! ```
//!
//! ## Start the clock
//!
...
impl<'a> Msi<'a> {
    ...
    pub fn enable(&self) -> Result<(), ErrorCode> {
        self.rcc.set_msi_enable(true);

        for _ in 0..100 {
            if self.rcc.is_msi_is_ready() {
                return Ok(());
            }
        }
...
pub mod tests {
    use super::{debug, ErrorCode, Msi, HSI_FREQUENCY_MHZ};
    ...
    pub fn run(msi: &Msi) {
        ...
        assert_eq!(Some(HSI_FREQUENCY_MHZ), msi.get_frequency_mhz());
