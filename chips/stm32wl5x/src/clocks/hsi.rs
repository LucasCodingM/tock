//! HSI (high-speed internal) clock driver for the STM32WL5x family. [^doc_ref]
//!
//! # Usage
//!
...
//! ```rust,ignore
//! let hsi = &peripherals.stm32wl5.clocks.hsi;
//! ```
//!
//! ## Start the clock
//!
...
impl<'a> Hsi<'a> {
    ...
    pub fn enable(&self) -> Result<(), ErrorCode> {
        self.rcc.set_hsi_enable(true);

        for _ in 0..100 {
            if self.rcc.is_hsi_is_ready() {
                return Ok(());
            }
        }
