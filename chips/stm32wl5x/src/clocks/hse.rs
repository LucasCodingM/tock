...
//! HSE (high-speed external) clock driver for the STM32WL5x family. [^doc_ref]
//!
//! # Usage
//!
...
//!
//! ```rust,ignore
//! hse.enable(stm32wl55jc::rcc::HseMode::BYPASS);
//! ```
//!
...
use crate::rcc::oscillator::HseMode;
...
impl<'a> Hse<'a> {
    ...
    pub fn enable(&self, source: HseMode) -> Result<(), ErrorCode> {
        if source == HseMode::BYPASS {
            self.rcc.set_hse_vddtcxo_output_enable(true);
        }

        self.rcc.set_hse_enable(true);

        for _ in 0..100 {
            if self.rcc.is_hse_is_ready() {
                return Ok(());
            }
        }
